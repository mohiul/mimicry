...
For this simulation the preys are heliconius butterfly and the representation of their wing pattern is with the help of cellular automata (CA). Every prey organism contain a binary genetic representation of CA which generates a fully developed pattern of size 16 by 16 bits from its initial state, with which the predator will identify the prey and store its level of palatability in memory. We choose CA as it can be easily represented with the help of a binary genome and evolutionary operations on the genomic representation, such as mutation and crossover can easily be applied. This 8 bit genome has a decimal range between 0 to 255, having each of this value associated to a unique CA pattern. In generating the pattern of figure \ref{fig:cellular-automata-rule-30}, the genetic representation would be the `New state of center cell'. When storing this pattern in Hopfield memory we take a linear representation of this 2-D pattern, while to consider similarity between two patterns we evaluate the hamming distance between their linear representation. 
...
\begin{tabular}{| p{1.75cm} | m{1.75cm} | p{1cm} | p{.5cm} | p{1.5cm} | p{.5cm} |}
  ...
  \multirow{2}{*}{Population} & Rule110 (Palatable) & \parbox[c]{2.1em}{\includegraphics[scale=0.40]{images/CARule110}} & 108 
  														& \multicolumn{2}{c|}{\multirow{2}{*}{10}} \\ \cline{2-4}
  					 									& {\parbox{1.75cm}{Rule30\\ (Unpalatable)}}& \parbox[c]{2.1em}{\includegraphics[scale=0.35]{images/CARule30}}  & 108 
  ...
  {\parbox{1.75cm}{Minimum\\ Attack Age}}	& \multicolumn{3}{c|}{} 						    & \multicolumn{2}{c|}{500} \\ \hline
