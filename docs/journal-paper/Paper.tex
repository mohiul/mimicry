...
\usepackage[paper=letterpaper,margin=0.9in]{geometry}
...
\title{A Model for the Evolution of Mimicry}
\author{Mohiul Islam\footremember{contactAuthor}{Contact Author} \footremember{address}{Faculty of Engineering and Computer Science, Concordia University, Montreal, Canada, H3G 1M8. moh\_i@encs.concordia.ca (M.I.), grogono@cse.concordia.ca (P.G.)} \qquad Peter Grogono\footrecall{address} \\
\mbox{}\\
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{images/Batesplate_ArM}
...
The \textsl{``FormAL framework"} is a collection of ideas and concepts taken from Peter Grogono's \gls{formal} project \citep{grogono2003} and are used to build a framework for modeling the evolution of mimicry. The primary goal of the FormAL project was ``\textit{To study the emergence of complexity}". While the principal behind it was ``\textit{not to include a variable in an agent unless the variable is genetically controlled (or, at least, genetically influenced)}". We will see that this principal was not completely established in simulating mimicry as it was not possible to come up with a suitable encoding for genetic representation of Hopfield Network which was used as a pattern recognizer for the predator species. 
...
The framework consists of a three dimensional environment where agents of the individual based simulation gets complete freedom of movement defined from their genetic representation. This toroidal cube can be visualized to provide intuition into the agents' behavior.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{images/cells-front}
...
Cellular automata are computer generated patterns applying very simple rules. First invented by Jon von Neuman and later extended by Stephen Wolfram \citep{Wolfram2002}, this field of study has created a lot of enthusiasm in the evolution of complexity with blocks of simplicity, in this case rules. We choose cellular automata as it can be easily represented with the help of a binary genome and then evolutionary operations on the genomic representation, such as mutation and crossover can easily be applied.

Every prey organism contain a binary genetic representation of CA which generates a fully developed pattern of size 16 by 16 bits from its initial state. With this pattern the predator will identify the prey and store its level of palatability in memory. This 8 bit genome has a decimal range between 0 to 255. Each of this value is associated to a unique CA pattern. In generating the pattern of figure \ref{fig:cellular-automata-rule-30}, the genetic representation would be the `New state of center cell'. To store in Hopfield memory we take a linear representation of this 2-D pattern and to find similarity between two patterns we calculate hamming distance between their linear representations. 

\begin{figure}[h!]
	\centering
	\includegraphics[resolution=300]{images/CARule30-large}
...
Franks and Noble have used different models to diversify species. In \citep{franks2002} they have used linear difference in number ``constrained to a 'ring' of values  from 1-20 (where 20 and 1 are neighbors)" to distinguish species diversity. The distance of one phenotype from another represents their level of similarity. 
...
The objective of a predator's interaction with prey is always to consume it. But based on the prey's pattern and palatability, the predator will either be able to consume it or throw it back to the environment. At this event the predator needs to learn the pattern with which the prey has been represented. The pattern represents palatability of the prey species, at least to the predator. To store this new pattern into memory, each predator is equipped with a Hopfield Network Memory. Every time a new interaction is made by the predator its memory is initialized with all the existing pattern that has already been encountered and the new one. The learning procedure used for this memory is Hebbian Learning \citep{hebb1949}. 
...
Movement behavior of a predator is calculated from its genome. The first 4 bits of the genome are converted from binary to decimal to determine the magnitude of force with which it will move towards the maximum crowd of prey present within its neighborhood. Number of bits are less than prey (6 bits) to reduce their maximum speed. So magnitude of movement of a predator varies within a range of 0-15 units. If no prey is present in the neighborhood then this force is active in trying to keep predators distributed all over the cells. A predator chooses the neighborhood cell which contains the least number of predators. When the neighborhood contains zero predators, it would select any one of them randomly and move towards that cell.
...
\centering
\setlength\tabcolsep{2pt}
\begin{tabular}{| p{3.5cm} | p{3.5cm} | p{1cm} | p{.5cm} | p{2.5cm} | p{1cm} |}
  \hline
   														&\multicolumn{3}{c|}{Prey configuration} 																	
   														& \multicolumn{2}{c|}{Predator configuration} \\ \hline
  \multirow{2}{*}{Population} & Rule110 (Palatable) & \parbox[c][2em][c]{2.3em}{\includegraphics{images/CARule110}} & 108 
  														& \multicolumn{2}{c|}{\multirow{2}{*}{10}} \\ \cline{2-4}
  					 									& Rule30 (Unpalatable)& \parbox[c][2em][c]{2.3em}{\includegraphics{images/CARule30}}  & 108 
...
\begin{figure*}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{images/ringSize10k-2Prey}
...
\begin{tabular}{|l|c|c|c|}
  ...
  Pattern & \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule60}} 
  				& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule61}} 
  				& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule62}}\\
  \hline
\end{tabular}
...
\begin{figure}[h]
	...
	\includegraphics[width=0.5\textwidth]{images/simTime7600}
...
\begin{tabular}{| p{3.5cm} | p{3.5cm} | p{1cm} | p{.5cm} | p{2.5cm} | p{1cm} |}
  \hline
   														&\multicolumn{3}{c|}{Prey configuration} 																	
   														& \multicolumn{2}{c|}{Predator configuration} \\ \hline
  \multirow{6}{*}{Population} & Rule110 (Palatable) & \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule110}} 
  																									& 150 & \multicolumn{2}{c|}{\multirow{6}{*}{30}} \\ \cline{2-4}
  					 									& Rule30  (Unpalatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule30}}  
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule55  (Palatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule55}}    
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule190 (Unpalatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule190}} 
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule57  (Palatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule57}}    
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule105 (Unpalatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule105}}& 150 & \multicolumn{2}{c|}{}\\ \hline
  \multirow{2}{*}{Reproduction} & Age Limit & \multicolumn{2}{c|}{100}  & \multicolumn{2}{c|}{500} \\ \cline{2-6}
...
\begin{figure}[h]
	\centering
	\includegraphics[resolution=300]{images/simTime10k-6Prey}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{images/ringSize10k-6Prey}
...
\begin{tabular}{| p{3.5cm} | p{3.5cm} | p{1cm} | p{.5cm} | p{2.5cm} | p{1cm} |}
  \hline
   														&\multicolumn{3}{c|}{Prey configuration} 																	
   														& \multicolumn{2}{c|}{Predator configuration} \\ \hline
  \multirow{4}{*}{Population} & Rule110 (Unpalatable) & \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule110}} 
  																									& 150 & \multicolumn{2}{c|}{\multirow{4}{*}{20}} \\ \cline{2-4}
  					 									& Rule30  (Unpalatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule30}}  
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule55  (Unpalatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule55}}    
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule190 (Unpalatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule190}}& 150 & \multicolumn{2}{c|}{}\\ \hline
  \multirow{2}{*}{Reproduction} & Age Limit & \multicolumn{2}{c|}{100}  & \multicolumn{2}{c|}{500} \\ \cline{2-6}
  						 									& Interval  & \multicolumn{2}{c|}{1000} & \multicolumn{2}{c|}{2000} \\ \hline
  \multirow{2}{*}{Mutation Rate} & Pattern   & \multicolumn{2}{c|}{0.05} & \multicolumn{2}{c|}{\multirow{2}{*}{0.3}} \\ \cline{2-4}
  						 									 & Genome    & \multicolumn{2}{c|}{0.5}  & \multicolumn{2}{c|}{} \\ \hline
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{images/ringSize8k-4Prey-unp}
...
\begin{figure}[h]
	\centering
	\includegraphics[resolution=300]{images/simTime6k-4Prey-unp-1-mem}
...
\begin{tabular}{| p{3.5cm} | p{3.5cm} | p{1cm} | p{.5cm} | p{2.5cm} | p{1cm} |}
  \hline
   														&\multicolumn{3}{c|}{Prey configuration} 																	
   														& \multicolumn{2}{c|}{Predator configuration} \\ \hline
  \multirow{4}{*}{Population} & Rule110 (Palatable) & \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule110}} 
  																									& 150 & \multicolumn{2}{c|}{\multirow{4}{*}{20}} \\ \cline{2-4}
  					 									& Rule30  (Palatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule30}}  
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule55  (Palatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule55}}    
  					 																				& 150 & \multicolumn{2}{c|}{}\\ \cline{2-4}
  					 									& Rule190 (Palatable)& \parbox[c][2em][c]{2.3em}{\includegraphics[resolution=300]{images/CARule190}}& 150 & \multicolumn{2}{c|}{}\\ \hline
  \multirow{2}{*}{Reproduction} & Age Limit & \multicolumn{2}{c|}{100}  & \multicolumn{2}{c|}{500} \\ \cline{2-6}
  						 									& Interval  & \multicolumn{2}{c|}{1000} & \multicolumn{2}{c|}{2000} \\ \hline
  \multirow{2}{*}{Mutation Rate} & Pattern   & \multicolumn{2}{c|}{0.05} & \multicolumn{2}{c|}{\multirow{2}{*}{0.3}} \\ \cline{2-4}
  						 									 & Genome    & \multicolumn{2}{c|}{0.5}  & \multicolumn{2}{c|}{} \\ \hline
...
\begin{figure}[h]
	\centering
	\includegraphics[resolution=300]{images/simTime10k-4Prey-p}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{images/ringSize10k-4Prey-p}
